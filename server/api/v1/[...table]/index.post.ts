  ...
  try {
      ...
      if (nameField in body && body[nameField]) {
        ...
        while (slugExists) {
          const existing = await db.select()
            .from(table)
            .where(eq(table.slug as any, finalSlug))
            .limit(1)
    ...
    for (const [fieldName, fieldValue] of Object.entries(body)) {
      if (fieldName in table && fieldValue !== null && fieldValue !== undefined) {
        ...
        // Check if field is a timestamp column by checking columnType
        if (field && field.columnType === 'PgTimestamp') {
    ...
    console.error('Error in POST:', error instanceof Error ? error.message : error)
    throw createError({
      statusCode: error.statusCode || 500,
      message: error.message || 'Error in POST request'
    })
  }
})